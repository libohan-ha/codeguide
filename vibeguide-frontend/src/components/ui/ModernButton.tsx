...
const ModernButton = memo(forwardRef<HTMLButtonElement, ModernButtonProps>(
  ({
    children,
    variant = 'primary',
    size = 'md',
    ...
    ), [disabled, fullWidth, floating, glow])
    ...
    ), [loading, loadingSpinner, Icon, iconPosition, size, children, glow, disabled])
    ...
    const finalClassName = useMemo(() => cn(
      ...
      variantClasses,
      className
    ), [baseClasses, size, variantClasses, className])
